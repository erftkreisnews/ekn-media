...
/*
 * Obergrenze für Backup-Speicher (Monitor + Cleanup). Ein vollständiges Projekt-Backup
 * liegt hier typischerweise bei ~12–20 GB; 5000 MB (Spatie-Default) war zu niedrig.
 */
$backupMaxStorageMb = max(1000, (int) env('BACKUP_MAX_STORAGE_MB', 20000));
$backupDestinationDisks = array_values(array_filter(array_map(
    static fn (string $disk): string => trim($disk),
    explode(',', (string) env('BACKUP_DESTINATION_DISKS', env('AWS_BUCKET') ? 's3' : 'local'))
)));
if ($backupDestinationDisks === []) {
    ...
    'monitor_backups' => [
        [
            ...
            'health_checks' => [
                ...
                \Spatie\Backup\Tasks\Monitor\HealthChecks\MaximumStorageInMegabytes::class => $backupMaxStorageMb,
            ],
        ],
