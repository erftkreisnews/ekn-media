...
@section('content')
    <div class="space-y-6">
        ...
        <form method="POST" action="{{ route('admin.backoffice.usage.update', $record) }}" class="bg-white rounded-lg border border-gray-200 shadow-sm p-6 space-y-6" id="usage-form-edit">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="news_item_id" name="news_item_id" class="mt-1 block w-full rounded-md border-gray-300 text-sm">
                        <option value="" @selected(old('news_item_id', $record->news_item_id) === null || old('news_item_id', $record->news_item_id) === '')>Freies Angebot</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="billing_type" name="billing_type" class="mt-1 block w-full rounded-md border-gray-300 text-sm">
                        ...
                        <option value="honorar" @selected(old('billing_type', $record->billing_type) === 'honorar')>Honorar (Audio)</option>
                    </select>
                </div>
            </div>
                ...
                <div>
                    ...
                    <div class="mt-1 grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <div>
                            ...
                            <input type="number" min="0" step="0.1" id="radio_minutes" name="radio_minutes" value="{{ old('radio_minutes', (float) ($record->radio_minutes ?? 0)) }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm">
                        </div>
                        <div>
                            <label for="print_copies" class="block text-xs font-medium text-gray-600">Print-Ausgabe / Auflage</label>
                            <input type="number" min="0" step="1" id="print_copies" name="print_copies" value="{{ old('print_copies', (int) ($record->print_copies ?? 0)) }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm">
                        </div>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">Video und Radio nicht in einem Eintrag kombinieren. Honorar (Audio): Abrechnungsart „Honorar“ und Radio-Sendeminuten.</p>
                </div>
            ...
            <div class="space-y-3">
                <label class="inline-flex items-start gap-2 text-sm text-gray-700">
                    <input type="checkbox" name="text_taken_over" value="1" class="mt-0.5 rounded border-gray-300 text-[#092E48] focus:ring-[#092E48]" @checked(old('text_taken_over', (bool) $record->text_taken_over))>
                ...
                <div>
                    ...
                    <textarea id="text_taken_over_excerpt" name="text_taken_over_excerpt" rows="4" class="mt-1 block w-full rounded-md border-gray-300 text-sm" placeholder="Optionaler Auszug oder Hinweis, welcher Text übernommen wurde …">{{ old('text_taken_over_excerpt', $record->text_taken_over_excerpt) }}</textarea>
                </div>
            </div>
        ...
        <div class="pt-2">
            <form
                ...
                data-delete-prompt="Diesen Nutzungs-Eintrag wirklich löschen? Geben Sie zur Bestätigung „ja“ ein."
            >
                @csrf
                @method('DELETE')
                <button
...
<script>
(() => {
    const form = document.getElementById('usage-form-edit');
    if (!form) return;
