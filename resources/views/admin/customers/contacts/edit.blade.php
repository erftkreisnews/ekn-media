...
<div class="py-6 max-w-2xl mx-auto px-4">
    <h1 class="text-2xl font-semibold text-gray-900">Kontakt bearbeiten</h1>
    <p class="mt-1 text-sm text-gray-600">{{ $customer->name }} · {{ $contact->name }}</p>
    ...
    <form method="POST" action="{{ url('/admin/customers/' . (int) $customer->id . '/contacts/' . $contact->id) }}" class="mt-6 bg-white rounded-lg border border-gray-200 p-6 space-y-4">
        @csrf
        @method('PUT')
        <div><label for="name" class="block text-sm font-medium text-gray-700">Name *</label><input type="text" name="name" id="name" value="{{ old('name', $contact->name) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#092E48] focus:ring-[#092E48]">@error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
        ...
        <div class="border-t border-gray-200 pt-4 mt-4">
            ...
            <label class="inline-flex items-center gap-2"><input type="checkbox" name="use_for_invoice" value="1" {{ old('use_for_invoice', $contact->use_for_invoice) ? 'checked' : '' }} class="rounded border-gray-300 text-[#092E48] focus:ring-[#092E48]"> Als Ansprechpartner für Rechnungen verwenden (bei Rechnungserstellung auswählbar)</label>
            ...
            <p class="mt-2 text-xs text-gray-500">Der Kontakt kann einer bestimmten Redaktion zugeordnet werden. Die Abrechnungsart hilft bei getrennten E-Mails für Lizenz und Honorar.</p>
        </div>
