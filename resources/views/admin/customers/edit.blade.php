...
@section('content')
    <div class="py-6">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ url('/admin/customers/' . $customer->id) }}" class="mt-6 bg-white rounded-lg border border-gray-200 shadow-sm p-6 space-y-4">
                @csrf
                <div>
                ...
                <div class="border-t border-gray-200 pt-4 mt-4">
                    ...
                    <p class="text-sm text-gray-600 mb-3">Einmal pro Medienhaus eintragen – gelten automatisch für alle Versandziele dieser Organisation (E-Mail, FTP/SFTP, Dateinamen, Sidecar).</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                            <label for="external_author_id" class="block text-sm font-medium text-gray-700">Autorennummer (author_id)</label>
                            <input type="text" name="external_author_id" id="external_author_id" value="{{ old('external_author_id', $customer->external_author_id) }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#092E48] focus:ring-[#092E48]">
                        </div>
                        <div>
                            <label for="external_supplier_id" class="block text-sm font-medium text-gray-700">Lieferanten-ID (supplier_id)</label>
                        ...
                        <div>
                            <label for="external_vendor_code" class="block text-sm font-medium text-gray-700">Vendor-Code (vendor_code)</label>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <a href="{{ url('/admin/customers/' . $customer->id) }}" class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md border border-gray-300 text-gray-700 bg-white hover:bg-gray-50">Zurück</a>
                </div>
            </form>
        </div>
    </div>
